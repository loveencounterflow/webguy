(function() {
  'use strict';
  var All_of, Any_of, Failure, Intertype, Isa, Iterator, Optional, Sentinel, XXX_nothing, _Intertype, _types, debug, defaults, get_value, props;
  ...
  Optional = class Optional extends Sentinel {
    ...
    get(r = null) {
      var R;
      R = this.value;
      this.value = XXX_nothing;
      if (R === XXX_nothing) {
        throw new Error("^Optional::get@1^ `Optional` instance is empty");
      }
      return r != null ? r : R;
    }
  ...
  //===========================================================================================================
  get_value = function(x, r = null) {
    if ((x instanceof Iterator) || (x instanceof Failure)) {
      return x.get(r);
    } else {
      return x;
    }
  ...
  _Intertype = class _Intertype {
    ...
    _validate(key, type, x) {
      ...
      // x = get_value x
      throw new Error(`^_Intertype::_validate@1^ expected a ${key}, got a ${this.type_of(x)}`);
    }
      ...
      cfg = {
        ...
        generator: function*({target, owner, key, descriptor}) {
          ...
          yield (function(key, type) {
            value = (x) => {
              return me._verify(key, type, x, isa);
            };
            descriptor = {...descriptor, value};
            return {
              target: me.verify,
